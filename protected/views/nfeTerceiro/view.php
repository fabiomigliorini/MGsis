...
$this->menu = array(
    ...
    array('label' => 'XML', 'icon' => ' icon-file', 'url' => MGSPA_API_URL . 'nfe-terceiro/' . $model->codnfeterceiro . '/xml', 'linkOptions' =>  array('target' => '_blank')),
...
<script type="text/javascript">
    ...
    function btnRevisaoClick(revisada) {
        bootbox.confirm("Tem Certeza?", function(result) {
            ...
            $.ajax({
                ...
                $.notify("Erro ao marcar revisão da NFe!", {
    ...
    function btnConferenciaClick(conferencia) {
        bootbox.confirm("Tem Certeza?", function(result) {
            ...
            $.ajax({
                ...
                $.notify("Erro ao marcar NFe como conferida!", {
                    position: "right bottom",
                    className: "error",
                    autoHideDelay: 15000
                });
    ...
    function marcarTipoProduto(codtipoproduto) {
        $.ajax({
    ...
    $(document).ready(function() {
        ...
        $('#buscar-form').on('submit', function(e) {
            ...
            if (barras == "") {
                $.notify("Preencha o código de barras ou referência!", {
                    position: "right bottom",
                    className: "error",
                    autoHideDelay: 15000
                });
            ...
            $.ajax({
                ...
                if (resp.items.length == 0) {
                    $.notify("Não localizei o código de barras " + barras + "!", {
                        position: "right bottom",
                        className: "error",
                        autoHideDelay: 15000
                    });
                    ...
                    $.notify("Localizei " + resp.items.length + " itens! Não sei qual você deseja!", {
                        position: "right bottom",
                        className: "error",
                        autoHideDelay: 15000
                    });
                ...
                $.notify("Erro ao localizar barras: " + error + "!", {
                    position: "right bottom",
                    className: "error",
                    autoHideDelay: 15000
                });
        ...
        $('#btnMarcarTodos').click(function(event) {
            const tipo = $('#NfeTerceiro_codtipoproduto').val();
            if (tipo == '') {
                bootbox.alert('Selectione o tipo de produto.')
                return;
            }
            console.log(tipo);
            bootbox.confirm("Tem certeza que deseja marcar todos os itens com o tipo selectionado?", function(result) {
                if (result) {
                    marcarTipoProduto(tipo);
                }
            });
        });
...
<?php if (!empty($model->NfeTerceiroItems)): ?>
    ...
    <?php if (!empty($model->podeEditar())): ?>
        <div class="input-group">
            <button class="btn" id="btnMarcarTodos">
                Marcar Todos Itens Como:
            </button>
            <?php
            echo MGActiveForm::select2(
                $model,
                'codtipoproduto',
                [
                    7 => 'Uso e Consumo',
