...
$this->pagetitle = Yii::app()->name . ' - Juntar Códigos de Barras';
$this->breadcrumbs=array(
	'Produtos'=>array('index'),
	$model->produto=>array('view','id'=>$model->codproduto),
	'Juntar Códigos de Barras',
);
...
<fieldset>
	...
	<div class="control-group ">
		<label class="control-label required" for="codprodutobarra">
			Manter <span class="required">*</span>
		</label>
		<div class="controls">
			<?php
	...
	<div class="control-group ">
		<label class="control-label required" for="codprodutobarra">
			Eliminar <span class="required">*</span>
		</label>
		<div class="controls">
			<?php
				echo CHtml::dropDownList(
					'Produto[codprodutobarraeliminar]', 
					isset($_POST['Produto']['codprodutobarraeliminar'])?$_POST['Produto']['codprodutobarraeliminar']:null, 
					$cods,
					array('class' => 'input-xlarge')					
				);
			?>
...
$(document).ready(function() {
	...
	$('#produto-form').submit(function(e) {
        ...
        e.preventDefault();
        bootbox.confirm('Tem certeza que deseja Juntar os códigos de barra selecionados? <br><br> <b class="text-error">Não será possível desfazer esta ação!</b>', function(result) {
            if (result) {
                currentForm.submit();
            }
        });
